    ...
    <div>
      ...
      {!loading && !error && page < totalPages && (
        <div style={{ textAlign: 'center', marginTop: 16, paddingBottom: 32 }}>
          <button className="btns" onClick={() => doSearch(query, page + 1, false)} disabled={loadingMore}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            {loadingMore
              ? <><span className="spinner" />Cargando más…</>
              : <>Cargar más <span style={{ color: 'var(--text3)', fontSize: 11 }}>(pág. {page + 1}/{totalPages})</span></>}
          </button>
          <div style={{ marginTop: 6, fontSize: 11, color: 'var(--text3)' }}>
            {allLogs.length.toLocaleString('es')} de {totalCount.toLocaleString('es')} resultados
          </div>
        </div>
      )}
    </div>
