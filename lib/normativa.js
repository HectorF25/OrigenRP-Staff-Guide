...
REGLA OC/MAFIA: Si el infractor pertenece a una organización criminal (OC), el tiempo de JAIL se DUPLICA para TODAS las infracciones de jail. El BAN no cambia nunca.

TABLA DE SANCIONES ADMINISTRATIVAS (tiempos normales / OC x2):
...
Jail 280 min (OC: 560): VDM masivo.
Jail 270 min (OC: 540): evadir soporte.
Jail 240 min (OC: 480): mentir en reporte; DM masivo; tirar de cable en rol.
Jail 120 min (OC: 240): robar vehículo de facción legal; dejar esposado a usuario.
Jail 90 min (OC: 180): pegar en reporte.
Jail 60 min (OC: 120): salirse de rol; usar armas de OC sin pertenecer; DM; IDP nula; falta fair-play; burlarse/insultar a abatido; forzar rol a policía sin motivo.
Jail 50 min (OC: 100): forzar rol de tiroteo; abuso de interacciones/animaciones; no valorar la vida.
Jail 40 min (OC: 80): PG; evadir rol; meterse en rol ajeno; forzar rol; forzar rol agresivo sin motivo; robar en zona segura; reportar sin motivo; omitir pérdida memoria PJ; no respetar entorno; AFK en entorno de rol; burlarse/insultar a abatido; RK.
Jail 30 min (OC: 60): no rolear choques/heridas; mal uso /me /do; mal uso /oop /ayuda /anon.
Jail 20 min (OC: 40): BJ (Bunny Jump); no tirar /entorno.
Jail 15 min (OC: 30): hablar mientras estás muerto.
CK admin: skin/ropa default; hacerse pasar por LSPD/EMS.

SANCIONES ESPECÍFICAS OC (warns y faltas — equipo de ilegales decide gravedad):
FALTAS LEVES OC (jail leve): forzar rol, no usar /entorno, ir enmascarados sin acto delictivo, excederse en robos/secuestros, incumplir normativa server, iniciar roles agresivos sin motivo, PG.
FALTAS MEDIAS OC (jail medio): no respetar rol entorno, dejar bridas por diversión, iniciar tiroteo policía sin rol previo, robar al mismo usuario constantemente, F8 en mitad rol/tiroteo, no valorar vida, falta FairPlay, vestimenta mafia por ciudad / paisano en roles agresivos, matar encima vehículos sin reposicionamiento.
FALTAS GRAVES OC (warn directo + jail grave): no respetar rol entorno sedes/barrios, alianzas con otras OC, robar armamento policial (+ BAN permanente), uso programas externos MG, mutilaciones/+18 no consentidos, faltar respeto staff, ignorar comunicados staff, propagar toxicidad, armas no permitidas por nivel dentro ciudad, mensajes de odio.
Cada 3 faltas individuales sin rectificar = warn a la organización.
...
- No usar /entorno antes de acto delictivo: jail 20 min (OC: 40).
- Robar en zona segura: jail 40 min (OC: 80).
- Robo a propia OC ("desertor"): CK ADMIN + BAN 7 DÍAS.
- Civiles sin OC NO pueden portar armas largas/automáticas para actos delictivos.

ARMAMENTO POR NIVEL OC:
- BANDAS LVL1: Cuchillo, SNS, 9mm, Vintage, MK2. LVL2: + Mini-SMG, TEC. LVL3: + Micro-SMG, AK-recortada.
- MAFIAS LVL1: Cuchillo, SNS, 9mm, Vintage, MK2. LVL2: cualquier pistola, Mini-SMG, Micro-SMG, TEC. LVL3: + AK-recortada, AK-47, Francotirador.
...
ZONAS SEGURAS: Comisaría, Hospital, Taller, Garaje, Ayuntamiento/INEM, Negocios, Discotecas, Casino, Concesionario, Armería, Gimnasio.
